class Solution:
    def findLadders(self, beginWord, endWord, wordList):
        ...
        map = [[0 for i in range(len(wordList))] for i in range(len(wordList))]
        ...
        while len(queue) != 0:
            u = queue.pop(0)
            for adj in range(n):
                if map[u][adj] == 1 and color[adj] != BLACK:
                    ...
                    if color[adj] == WHITE:
                        ...
                        if if_search and dis[adj] == dis[u] + 1:
                            if color[u] == GRAY:
                                parent[adj].append(u)
                            else:
                                parent[u].append(adj)
