...
class Solution:
    ...
    def networkDelayTime(self, times: List[List[int]], N: int, K: int) -> int:
        ...
        while heap:
            ...
            # 如果有了这一步，可以不需要heap，queue就行了
            if visited[node] and visited[node] <= d:
                continue
            visited[node] = d if d else -1
            print(node, d)
            n += 1
            for neighbor in graph[node]:
                if neighbor != K and not visited[neighbor]:
                # if not visited[neighbor]:
                    # if value[node, neighbor] + d < visited[neighbor]:
                    heapq.heappush(heap, (value[node, neighbor] + d, neighbor))
        if n == N:
            return max(visited)
        return -1
